...
AgIA incident response contains two agent nodes connected in a `StateGraph`. Both agents share a typed state and communicate only through structured Pydantic models.
...
|---|---|
| `remediate_vulnerability` | Simulates a sandboxed CVE remediation flow against a validated asset IP |

To add a new tool, follow the protocol in [security.md](security.md#protocol-for-adding-a-new-tool).